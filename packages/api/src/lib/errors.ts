...
export const errors = {
   ...
   */
  unauthorized: (message?: string) => new TRPCError({
    code: 'UNAUTHORIZED',
   ...
   * Resource not found
   */
  notFound: (entity: string) => new TRPCError({
    code: 'NOT_FOUND',
   ...
   * Permission denied
   */
  forbidden: (message?: string) => new TRPCError({
    code: 'FORBIDDEN',
   ...
   */
  conflict: (message: string) => new TRPCError({
    code: 'CONFLICT',
    message
  }),
   ...
   * Invalid input
   */
  badRequest: (message: string) => new TRPCError({
    code: 'BAD_REQUEST',
    message
  }),
   ...
   */
  internal: (message?: string) => new TRPCError({
