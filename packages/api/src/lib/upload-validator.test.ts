...
describe("Upload Validator", () => {
  ...
  describe("validateFileSize", () => {
    ...
    it("should reject files exceeding size limit", () => {
      ...
      expect(() => validateFileSize(mockFile, 512 * 1024)).toThrow(
        "File size exceeds"
      );
    });
    ...
    it("should reject empty files", () => {
      ...
      expect(() => validateFileSize(emptyFile, 2 * 1024 * 1024)).toThrow(
        TRPCError
      );
      expect(() => validateFileSize(emptyFile, 2 * 1024 * 1024)).toThrow(
        "cannot be empty"
  ...
  describe("validateFileType", () => {
    ...
    it("should reject disallowed MIME types", () => {
      ...
      expect(() => validateFileType(mockFile, allowedTypes)).toThrow(
        "not allowed"
      );
    });
  ...
  describe("validateFileExtension", () => {
    it("should allow allowed extensions", () => {
      ...
      expect(() =>
        validateFileExtension(mockFile, allowedExtensions)
      ).not.toThrow();
    });

    it("should reject disallowed extensions", () => {
      const allowedExtensions = [".png"];
      expect(() => validateFileExtension(mockFile, allowedExtensions)).toThrow(
        TRPCError
      );
    });
  ...
  describe("validateFileUpload", () => {
    ...
    it("should reject file that fails any check", async () => {
      ...
      await expect(validateFileUpload(file, options)).rejects.toThrow(
        TRPCError
      );
    });
  ...
  describe("isImageFile", () => {
    it("should identify image files by MIME type", () => {
      expect(
        isImageFile({ name: "test.jpg", size: 1000, type: "image/jpeg" })
      ).toBe(true);
      expect(
        isImageFile({ name: "test.png", size: 1000, type: "image/png" })
      ).toBe(true);
    });
    ...
    it("should return false for non-images", () => {
      expect(
        isImageFile({ name: "test.pdf", size: 1000, type: "application/pdf" })
      ).toBe(false);
      expect(
        isImageFile({
          name: "test.docx",
  ...
  describe("isDocumentFile", () => {
    ...
    it("should return false for non-documents", () => {
      expect(
        isDocumentFile({ name: "test.jpg", size: 1000, type: "image/jpeg" })
      ).toBe(false);
      expect(
        isDocumentFile({ name: "test.mp4", size: 1000, type: "video/mp4" })
      ).toBe(false);
    });
  });
  ...
  describe("UploadPresets", () => {
    it("should have correct profile image preset", () => {
      ...
      expect(UploadPresets.PROFILE_IMAGE.allowedMimeTypes).toContain(
        "image/jpeg"
      );
    ...
    it("should have correct document preset", () => {
      ...
      expect(UploadPresets.DOCUMENT.allowedMimeTypes).toContain(
        "application/pdf"
      );
    });
  });
