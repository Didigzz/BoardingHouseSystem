...
export const createPaymentRouter = (protectedProcedure: ProtectedProcedure) => {
  return {
    getAll: protectedProcedure
      ...
      .handler(
        async ({
          ...
          ctx: { db: PrismaClientType };
          input?: unknown;
        }) => {
          // Initialize dependencies
          const paymentRepository = new PrismaPaymentRepository(ctx.db);
          const listPaymentsHandler = new ListPaymentsHandler(
            paymentRepository
          );

          // Execute query
          const payments = await listPaymentsHandler.handle(
            input as
              | {
    ...
    create: protectedProcedure
      ...
      .handler(
        async ({
          ...
          // Execute command
          const payment = await createPaymentHandler.handle(
            input as {
              boarderId: string;
              amount: number;
              type: "RENT" | "UTILITY" | "DEPOSIT" | "OTHER";
    ...
    markAsPaid: protectedProcedure
      ...
      .handler(
        async ({
          ...
          const paymentRepository = new PrismaPaymentRepository(ctx.db);
          const paymentService = new PaymentService(paymentRepository);
          const markPaymentPaidHandler = new MarkPaymentPaidHandler(
            paymentRepository,
            paymentService
          );

          // Execute command
          const payment = await markPaymentPaidHandler.handle({
            ...inp,
            paidDate: inp.paidDate ?? new Date(),
          });
    ...
    getStats: protectedProcedure.handler(
      async ({ ctx }: { ctx: { db: PrismaClientType } }) => {
        // Initialize dependencies
        const paymentRepository = new PrismaPaymentRepository(ctx.db);
        const getPaymentStatsHandler = new GetPaymentStatsHandler(
          paymentRepository
        );
    ...
    getMonthlyRevenue: protectedProcedure
      ...
      .handler(
        async ({
          ...
          // Initialize dependencies
          const paymentRepository = new PrismaPaymentRepository(ctx.db);
          const getMonthlyRevenueHandler = new GetMonthlyRevenueHandler(
            paymentRepository
          );
