...
export const createDashboardRouter = (protectedProcedure: Procedure) => {
  return createTRPCRouter({
    getStats: protectedProcedure.query(
      async ({ ctx }: AuthenticatedCtx): Promise<DashboardStats> => {
        const [
          totalRooms,
          availableRooms,
          occupiedRooms,
    ...
    getRecentActivity: protectedProcedure.query(
      async ({ ctx }: AuthenticatedCtx): Promise<Activity[]> => {
    ...
    getUpcomingPayments: protectedProcedure.query(
      async ({ ctx }: AuthenticatedCtx) => {
