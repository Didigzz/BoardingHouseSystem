 ...
 */
export function getPaymentsDueWithin(
  payments: Payment[],
  days: number
): Payment[] {
  ...
  return payments.filter(
    (payment) => payment.status === "PENDING" && payment.dueDate <= targetDate
  );
}
...
export function filterPayments(
  ...
  return payments.filter((payment) => {
    ...
    if (filters.search) {
      ...
      const boarderName =
        `${payment.boarder.firstName} ${payment.boarder.lastName}`.toLowerCase();
