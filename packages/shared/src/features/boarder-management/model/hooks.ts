...
export function useBoardersByOccupancy(boarders: BoarderWithRoom[]) {
  return useMemo(() => {
    ...
    return {
      ...
      withoutRoom,
      occupancyRate:
        boarders.length > 0 ? (withRoom.length / boarders.length) * 100 : 0,
    };
  }, [boarders]);
}
 ...
 */
export function useRecentBoarders(
  boarders: BoarderWithRoom[],
  daysBack: number = 30
) {
  return useMemo(() => {
 ...
 */
export function useSearchBoarders(
  boarders: BoarderWithRoom[],
  searchTerm: string
) {
  return useMemo(() => {
